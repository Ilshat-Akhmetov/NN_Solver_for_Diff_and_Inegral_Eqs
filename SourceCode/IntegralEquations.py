...
class IntegralEquations:
    ...
    def calculateFredholmEquation1D(integral_function: Callable,
        ...
        int_nodes, dx = IntegralEquations.get1DcentralrectangularNodes(left_bound, right_bound, n_points)
        total_val = torch.zeros(curr_val.shape)
        n = len(curr_val)
        for i in range(n):
            total_val[i][0] = torch.sum(integral_function(curr_val[i], int_nodes, nn_model)) * dx
        return total_val
    ...
    def calculateVolterraEquation1D(integral_function: Callable,
        ...
        n = len(curr_val)
        for i in range(n):
            total_val[i][0] = torch.sum(integral_function(curr_val[i], int_nodes[:i+1], nn_model)) * dx
        return total_val
